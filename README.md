...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  > actions sends out the command for the state to be updated, reducers control how the the state will be updated and store, keeps and maintains all version of the state.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
  > Application state refers to the state that is global and accessible across every part of the application, while component state referes to that which is local to that component.
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
  > redux-thunk is a redux middleware, it helps us to make redux process asynchronous because redux by default is synchronous.
